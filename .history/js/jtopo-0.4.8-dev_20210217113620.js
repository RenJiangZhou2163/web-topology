!(function (window) {
              ...
              if (a["nodeSrc"] && a["nodeDst"]) {
                nodes.forEach(function (nodeEle) {
                  if (nodeEle.elementType == "node") {
                    if (nodeEle.nodeId == a["nodeSrc"]) nodeA = nodeEle;
                    if (nodeEle.nodeId == a["nodeDst"]) nodeZ = nodeEle;
                  }
                });
              }
  ...
  (function (JTopo) {
    ...
    function genImageAlarm(a, b) {
      ...
      try {
        ...
        var c = new Image();
        return (
          (c.src = changeColor(graphics, a, b)), (alarmImageCache[a.src] = c), c
        );
      } catch (d) { }
      return null;
    }
    ...
    function getOffsetPosition(a) {
      ...
      if ("getBoundingClientRect" in document.documentElement)
        var d = a.getBoundingClientRect(),
          ...
          b =
            d.top + (self.pageYOffset || (g && g.scrollTop) || f.scrollTop) - h,
          c =
            d.left +
            (self.pageXOffset || (g && g.scrollLeft) || f.scrollLeft) -
            i;
      else
        do
          (b += a.offsetTop || 0),
      ...
      function initJtopoEvent(b) {
        ...
        JTopo.util.isIE || !window.addEventListener
          ...
          (window.addEventListener(
            "keydown",
            function (b) {
              stage.dispatchEventToScenes(
                "keydown",
                JTopo.util.cloneEvent(b)
              );
              var c = b.keyCode;
              (37 == c || 38 == c || 39 == c || 40 == c) &&
                (b.preventDefault
  ...
  (function (a) {
      ...
      if (currNode.layout && currNode.layout.on) {
        ...
        if ("circle" == d) {
          ...
          if (currNode.selectedLocation) {
            //纪录节点最后一次移动完成时的坐标
            editor.lastMovedX =
              currNode.selectedLocation.x + event.dx - currNode.x;
            editor.lastMovedY =
              currNode.selectedLocation.y + event.dy - currNode.y;
          }
          //子节点同步移动
    ...
    function getNodeChilds(scene, rootNode) {
      for (var d = [], e = 0; e < scene.length; e++)
        scene[e] instanceof a.Link &&
          scene[e].nodeA === rootNode &&
  ...
  (function (a) {
    ...
    function d(a, b, c) {
      return function (d) {
        for (let e = 0; b > e; e++) a(), c && d.turn(c), d.move(3);
      };
    }
    ...
    function e(a, b) {
      let c = 2 * Math.PI;
      return function (d) {
        for (let e = 0; b > e; e++) a(), d.turn(c / b);
      };
    }

    function f(a, b, c) {
      return function (d) {
        for (let e = 0; b > e; e++) a(), d.resize(c);
      };
    }
