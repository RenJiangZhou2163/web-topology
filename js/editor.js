...
propertyPanel.prototype.refreshPanel = function(
  ...
  if (e && d && d.length > 0) {
    var i = 0;
    for (i = 0; i < d.length; i++) {
      e.accordion('getPanel', d[i]['title']).
          panel('refresh',
              d[i]['url'] + '?templateId=' + templateid + '&moduleId=' +
              moduleId + '&envTemplateId=' + editor.templateId);
    }
...
networkTopologyEditor.prototype.initMenus = function() {
  ...
  self.nodeTextMenu.on('mouseover', function(e) {
    ...
    if ('调整节点文字位置' == text) {
      //处于边界时三级菜单位置调整
      var menuX = parseInt(this.style.left) +
          $(document.getElementById('justfyNodeText')).width();
      if (parseInt(this.style.left) <
          parseInt(document.getElementById('nodeMainMenu').style.left)) {
        menuX = parseInt(this.style.left) -
            $(document.getElementById('justfyNodeText')).width();
      }
      self.nodeTextPosMenu.css({
        top: parseInt(this.style.top) +
            $(document.getElementById('justfyNodeText')).height(),
        left: menuX,
      }).show();
    } else {
      self.nodeTextPosMenu.hide();
  ...
  this.scene.mouseup(function(e) {
    ...
    if (e.button == 2) {  // 右键菜单
      ...
      if (e.target != null && e.target instanceof JTopo.Node &&
        ...
        } else if (e.target && e.target instanceof JTopo.Node &&
            self.beginNode !== e.target) {//结束连线
...
editor.utils = {
  ...
  saveNodeNewState: function() {
    ...
    if (editor.currentNode.historyStack.length >=
        editor.currentNode.maxHistoryStep + 1) {
  ...
  saveNodeInitState: function() {
    if (!editor.currentNode.hasInitStateSaved) {
      editor.currentNode.historyStack.push(
          JTopo.util.clone(editor.currentNode));
