...
function propertyPanel(mainControlDiv) {
    ...
    this.props = {
        'EC': [
            {'title': '交换机属性', 'url': rootPath + 'switchesManage/showSwitches'},
        ],
        'FW': [
            {
                'title': '防火墙出口规则集',
                'url': rootPath + 'routerManage/showFireWallEgressRules',
            },
            {
                'title': '防火墙入口规则集',
                'url': rootPath + 'routerManage/showFireWallInRules',
            }],
        'VR': [
            {'title': '路由器属性', 'url': rootPath + 'routerManage/showRouter'},
            /*{"title": "路由器连接信息", "url": rootPath + "routerManage/showRouterConnect"},*/
            {'title': '路由规则集', 'url': rootPath + 'routerManage/showRouterRules'},
            /* {"title": "网络访问规则集", "url": rootPath + "routerManage/showNetworkAclRules"}, */
            /* {"title":"端口转发规则","url":rootPath+"vr-portforwardrule.html"},*/
            {
                'title': '网络转发规则集',
                'url': rootPath + 'routerManage/showPortForwardRules',
            },
            /* {"title":"虚拟路由器类型","url":rootPath+"vr-networktype.html"}*/
        ],
        'VM': [
            {'title': '虚拟机属性', 'url': rootPath + 'templateManage/showVmTemplate'},
        ],
        'ECVR': [
            {'title': '路由器网络属性', 'url': rootPath + 'vmvrManage/showVmvrRouter'},
            {'title': '子网属性', 'url': rootPath + 'vmvrManage/showVmvrSwitch'},
 ...
 * @param 自定义路由器的节点实例
 */
propertyPanel.prototype.createNewPanels = function (
    ...
    if (d) {
        ...
        for (i = 0; i < d.length; i++) {
            c.control.accordion('add', {
                ...
                selected: false,
                method: 'post',
                href: d[i]['url'] + '?templateId=' + templateid + '&moduleId=' +
            ...
            if (i > 0)
                e.panel('refresh',
                    d[i]['url'] + '?templateId=' + templateid + '&moduleId=' +
                    moduleId + '&envTemplateId=' + editor.templateId);
        }
        //打开第一个面板
        ...
        if (node instanceof JTopo.Node && node.dataType == 'ECVR') {
            ...
            if (connectedDevice.length > 0) {
                for (var j = 0; j < connectedDevice.length; j++) {
                    this.createPanel(editor, templateid,
                        connectedDevice[j].nodeZ.text + '--路由器连接信息', false,
                        connectedDevice[j].nodeZ.nodeId,
                        connectedDevice[j].nodeZ.templateId, connectedDevice[j].id,
                        rootPath + 'vmvrManage/showVmvrRouterConnect', true);
                }
            }
        }
    }
...
propertyPanel.prototype.refreshPanel = function (
    ...
    if (e && d && d.length > 0) {
        var i = 0;
        for (i = 0; i < d.length; i++) {
            e.accordion('getPanel', d[i]['title']).panel('refresh',
                d[i]['url'] + '?templateId=' + templateid + '&moduleId=' +
                moduleId + '&envTemplateId=' + editor.templateId);
        }
    }
 ...
 * @param name 面板名字
 * @param selected 是否选中
 * @param targetnodeId 指向的
 * @param templateId 虚拟路由器指向的设备ID
 * @param moduleId nodeId
 * @param uri 页面地址
 * @param isShow 是否立即显示新增的面板
 */
propertyPanel.prototype.createPanel = function (
    editor, vmvrId, name, selected, targetnodeId, templateId, moduleId, uri,
    isShow) {
    ...
    if (isShow) {
        ...
        e.panel('refresh',
            uri + '?vmvrId=' + vmvrId + '&templateId=' + templateId + '&moduleId=' +
    ...
    a['ECVR'].push({
        'title': name,
        'url': uri + '?vmvrId=' + vmvrId + '&templateId=' + templateId +
            '&moduleId=' + moduleId + '&envTemplateId=' + editor.templateId +
            '&title=' + name + '&size=' + size,
        'targetNetwork': targetnodeId,
    });
};
 ...
 * 为减少请求参数长度,进行了字符串的替换
 */
propertyPanel.prototype.saveTopology = function (showAlert) {
    ...
    if (topologyJSON) {
        for (var key in this.jsonCode) {//字符压缩
            topologyJSON = topologyJSON.replace(new RegExp('"' + key + '"', 'gm'),
                this.jsonCode[key]);
        }
    ...
    $.ajax({
        url: rootPath + 'topologyManage/saveTopologyJSON',
        async: true,
        type: 'POST',
        dataType: 'json',
        data: {
        ...
        success: function (response) {
            ...
            if (err && err != 'ok') {
                ...
                self.replaceStage(editor.templateId, editor.topologyId, showAlert,
                    editor.stage.topoLevel);
                self.closeLoadingWindow();
            }
        },
    });
...
propertyPanel.prototype.deleteNodeById = function (id, type, dataType) {
    ...
    var self = this;
    $.ajax({
        url: rootPath + 'topologyManage/deleteNodeById',
        async: false,
        type: 'POST',
        dataType: 'json',
        data: {
 ...
 */
propertyPanel.prototype.resetPanelProperty = function (
    title, propTitle, index, templateId) {
    ...
    if (cuurPanel) {
        if (title.indexOf('路由器连接信息') > 0) {
            ...
            cuurPanel.panel('refresh',
                d[index]['url'] + '?templateId=' + templateId + '&moduleId=' +
                moduleId + '&envTemplateId=' + envTemplateId);
        }
    }
...
function networkTopologyEditor(mainControl) {
    ...
    this.config = {
        ...
        containerFillColor: '243,234,195',          // 容器填充颜色
        containerShadow: false,                     //
        containerShadowColor: 'rgba(0,0,0,0.5)',    //
        containerFont: '20px Consolas',             // 容器文字大小
...
networkTopologyEditor.prototype.initMenus = function () {
    ...
    self.nodeTextMenu.on('mouseover', function (e) {
        ...
        if ('调整节点文字位置' == text) {
            ...
            self.nodeTextPosMenu.hide();  // 隐藏三级菜单
        }
    });

    // 节点文字-调整节点文字位置-三级菜单事件处理
    self.nodeTextPosMenu.on('click', function (event) {

        let text = $.trim($(event.target).text());  // 菜单文字

        if (self.currentNode && self.currentNode instanceof JTopo.Node) {

            self.utils.saveNodeInitState();  // 保存变更前状态
    ...
    self.groupAlignMenu.on('click', function (e) {
        ...
        if (!currNode || !selectedNodes || selectedNodes.length == 0)
            return;
        $(this).hide();
        var text = $.trim($(e.target).text());

        selectedNodes.forEach(function (n) {
            if (n.nodeId == currNode.nodeId)
                return true;
            if (text == '水平对齐') {
                n.y = currNode.y;
            } else if (text == '垂直对齐') {
        ...
        $('div[id$=\'Menu\']').hide();  // CHANGE:事件处理完成后，隐藏右键菜单
    });
    ...
    self.groupMangeMenu.on('mouseover', function (e) {
        ...
        if (text == '对齐方式') {
            ...
            var menuX = parseInt(this.style.left) +
                $(document.getElementById('alignGroup')).width();
            ...
            self.groupAlignMenu.css({
                top: parseInt(this.style.top) +
                    $(document.getElementById('alignGroup')).height(),
                left: menuX,
            }).show();
        } else {
    ...
    self.containerMangeMenu.on('click', function (e) {
        ...
        let text = $.trim($(e.target).text());  // 获取菜单选项
    ...
    // 菜单-容器管理菜单-容器描述文字
    self.containerMangeMenu.on('mouseover', function (e) {
        // 隐藏第三级菜单
        self.nodeTextPosMenu.hide();
        ...
        if (e.button == 2) {
            ...
            if (e.target) {
                ...
                if (e.target instanceof JTopo.Node) {
                    let selectedNodes = self.utils.getSelectedNodes();  // 获取选择的节点

                    // 如果是节点多选状态弹出(新建分组、对齐方式)菜单
            ...
            if (e.target != null && e.target instanceof JTopo.Node && !self.isSelectedMode) {
                ...
                if (self.beginNode == null) {
                    ...
                    // self.link.strokeColor = JTopo.util.randomColor();  新建链路颜色随机
                    ...
                    //     self.createPanel(self, self.beginNode.templateId,
                    //         endNode.text + '--路由器连接信息', true, endNode.nodeId,
                    //         endNode.templateId, self.beginNode.id,
                    //         rootPath + 'vmvrManage/showVmvrRouterConnect', false);
                    //   }
                    // }
                    ...
                    // 连线所处拓扑层级
                    link.topoLevel = editor.stage.topoLevel;
                    link.parentLevel = $('#parentLevel').val();
    ...
    $(document).keydown(function (e) {
        if (e.shiftKey) {//组合键模式
            switch (e.which) {
                ...
                case  70:
                    //ctrl+f 全屏显示
                    editor.utils.showInFullScreen(editor.stage.canvas,
                        'RequestFullScreen');
                    //return false;
                    break;
                case  72:
    ...
    drawArea.ondrop = function (e) {
        ...
        if (data) {
            ...
            if (datas && datas.length == 3) {
                ...
                node.setBound(
                    (e.layerX ? e.layerX : e.offsetX) - self.scene.translateX -
                    self.config.nodeDefaultWidth / 2,
                ...
                //节点所属层次
                node.topoLevel = parseInt(
                    $('#selectLevel').find('option:selected').val());
                //节点所属父层次
...
editor.utils = {
    ...
    toMerge: function () {
        ...
        container.fillColor = editor.config.containerFillColor;
    ...
    cloneNode: function (n) {
        if (n instanceof JTopo.Node) {
            ...
            newNode.borderRadius = editor.config.nodeBorderRadius;
            ...
            //newNode.nodeId = "device" + deviceNum;
            newNode.nodeId = '' + new Date().getTime() * Math.random();
    ...
    addNodeText: function (x, y) {
        ...
        console.log(editor.currentNode.text);
    ...
    reMakeNodeAction: function () {
        if (editor.currentNode.currStep >= editor.currentNode.maxHistoryStep || editor.currentNode.currStep >= editor.currentNode.historyStack.length - 1)
            return;
        editor.currentNode.currStep++;
    ...
    saveNodeNewState: function () {
        console.log(editor.currentNode.historyStack);
    ...
    getNode: function (id) {
        var node;
        $.ajax({
            url: rootPath + 'topologyManage/getNode',
            async: false,
            type: 'POST',
            dataType: 'json',
            data: {
    ...
    saveNode: function (id, nextLevel) {
        var ok = false;
        $.ajax({
            url: rootPath + 'topologyManage/saveNode',
            async: false,
            type: 'POST',
            dataType: 'json',
            data: {
